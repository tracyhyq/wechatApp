	...
	public function sendMessage() {
		if(!empty( $this->keyword )) {	
            if(strpos($this->keyword, "天气") !== false){
        		...
        		$title = "爱情岁月--郑伊健";
        		$description = "爱情岁月--郑伊健 在线试听,MP3免费下载,爱情岁月--郑伊健 歌词下载_百度音乐-听到极致";
        		$musicUrl = "http://weixincourse-weixincourse.stor.sinaapp.com/mysongs.mp3";
        		$hQMusicUrl = "http://weixincourse-weixincourse.stor.sinaapp.com/mysongs.mp3";
        		$thumbMediaId = "";
	...
	public function sendMessage() {
	  	...
	  	if($customEvent == "subscribe"){
			...
			MessageUtil::sendOneTextImageMessage($this->fromUsername, $this->toUsername,$title,$description, $picUlr,$url);
		  	...
		  	switch ($eventKey){
		  		case "key_dpdz" :
					$message = "<a href ='http://api.map.baidu.com/geocoder?address=北京立水桥&output=html&src=hanfei|shuiguo'>我的位置</a>";
		  			MessageUtil::sendTextMessage($this->fromUsername, $this->toUsername,  $message);
					break ;
		  		case  "key_rxsg" : 
		    		...
		    		$title = "热销水果信息";
		  			...
		  			break ;
		  		case "key_lxdh": 
		  			$content = "contact Phone Number: 13883838888.";
		  			MessageUtil::sendTextMessage($this->fromUsername, $this->toUsername, $content);
		  			break;
		  		case "key_yhxx": 
		  			$content = "夏季优化信息：慢100减少20元；\n满200元减50元。";
		  			MessageUtil::sendTextMessage($this->fromUsername, $this->toUsername, $content);
		  			break;
		  		case "key_tqcx":
		  			$message = "请回复格式：\n 天气+地点,例如:天气北京";
